...
## Saving flows so I know what was used in model runs. Names included so can calculate flowstats
saveRDS(dr.ne.scenarios.list, 'modelresults/flows_used.rds')

## When using natural flow data, just pull it out from the list here
#flowdata <- all.scenarios.list$natural.flow

count <- 5 # how many years to project ## Prob go for 250
iterations <- 5 # number of replicate projections to run (mid loop) ## Prob go for 500
...
## List to fill in all of the means.list values for each sppmod
## This will be all results combined into a 2-level nested list
sppmod.means.list <- list()
sppmod.flowmeans.list <- list()
...
foreach(sppmod = names(allcombos)) %dopar% { 
    ...
    ## list to fill in with means of spp and their sds and ses for each year
    ## each df in list is one flow mod setting
    means.list <- list()
...
### ----------------------------------------------------------------------------
    for(flowmod in names(dr.ne.scenarios.list)) {
        
        flowdata <- dr.ne.scenarios.list[[flowmod]]
...
### ----------------------------------------------------------------------------
### End of outer flow loop #####################################################
### ----------------------------------------------------------------------------

    ## Save the objects as .rds files - then use loadRDS in other file. 
    saveRDS(means.list, paste0('modelresults/', sppmod, '.rds'))

    sppmod.means.list[[sppmod]] <- means.list
    sppmod.flowmeans.list[[sppmod]] <- flowmeans.list

}

### ----------------------------------------------------------------------------
### End of outer spp selection loop ############################################
### ----------------------------------------------------------------------------


saveRDS(sppmod.means.list, 'modelresults/sppmod_flowmod_results.rds')
saveRDS(sppmod.flowmeans.list, 'modelresults/sppmod_flowmod_flowmeans_results.rds')
...
## ## Plotting
## rel.abund.trends <- ggplot(sppmod.means.list$seven$drought_1, aes(year,
##                                       mean.rel.abund,
##                                       colour = species,
##                                       fill = species)) +
